...
export class TokrFiStack extends cdk.Stack {
	constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
		super(scope, id, props);

		// Get the hosted zone.
		const hostedZone = route53.HostedZone.fromHostedZoneAttributes(this, "TokrFiZone", {
			hostedZoneId: cdk.Fn.importValue("TokrFiHostedZoneId"),
			zoneName: cdk.Fn.importValue("TokrFiHostedZoneName"),
		});
		...
		const siteBucket = new s3.Bucket(this, "TokrFiBucket", {
			bucketName: "tokr-fi-bucket-" + this.account + "-" + this.region,
		...
		const originAccessIdentity = new cloudfront.OriginAccessIdentity(this, "OAI", {
			comment: "created_for_tokr_fi_bucket",
		});
		siteBucket.grantRead(originAccessIdentity);

		// Create the ssl cert to be used with cloudfront and the hosted zone. Note this MUST be created in us-east-1
		const siteCertificate = new acm.DnsValidatedCertificate(this, "TokrFiSiteCertificate", {
			hostedZone,
		...
		const siteDistribution = new cloudfront.CloudFrontWebDistribution(this, "TokrFiDist", {
			originConfigs: [
				{
					s3OriginSource: {
					...
					behaviors: [
						{
							...
							forwardedValues: {
								queryString: false,
								headers: ["X-Prerender-Token", "X-Prerender-Host", "X-Prerender-Cachebuster"],
							},
						},
					],
				},
			...
			errorConfigurations: [
				{ errorCode: 400, responsePagePath: "/index.html", responseCode: 200 },
		...
		new route53.ARecord(this, "TokrFiStackAliasRecord", {
			zone: hostedZone,
